...
yield_stress    = 135*Pini_Pappl*1e6;
...
Ch_ti_kinetic       = 1/5 * 1/3*Ch_ti_fluid_dif_phi;
Ch_ti_deformation   = 1/2 * 1.0*Ch_ti_fluid_dif_phi;
eb                  = 1/Ch_ti_deformation;
Da                  = eb/(P_ini/eta_mat);   % Re-scaling of Da (LAMBDA_4)
% Numerical resolution
nx              = 151;                              % Numerical resolution width
...
% end
%------------------------
PERT            = (2.5*phi_ini-phi_ini) * exp(-(X2D/rad_a).^2 -(Y2D/rad_b).^2);
PHI             = PHI_INI + PERT;
%==========================================================================
...
VX              =  eb*Y2D_vx;                   % Simple shear, shearing in x
VY              =   0*Y2D_vy;                   % Simple shear, zero velocity in y
...
while time < time_tot
    if ires==1
        load RES_HMC_atg_00002; ires=0;
        %RES_HMC_atg_restart.mat; 
    end
...
%         TII                         = sqrt( 1/4.*(TAUXX-TAUYY).^2 + swell2(swell2(TAUXY,1),2).^2);
        TII                         = sqrt( (TAUXX.^2 + TAUYY.^2)/2 + swell2(swell2(TAUXY,1),2).^2);
        % === PLASTICITY
        J2                    = TII;        
        lam                   = (1-yield_stress./TII); lam(lam<0) = 0;
        if sum(lam(:)>0)>0
            TAUXX  = (1-lam).*TAUXX;
            TAUYY  = (1-lam).*TAUYY;
            lam_av = (lam(2:end,:)    + lam(1:end-1,:))/2;
            lam_av = (lam_av(:,2:end) + lam_av(:,1:end-1))/2;    % Averaged viscosity for shear stress
            TAUXY  = (1-lam_av).*TAUXY;
            TII    = sqrt( (TAUXX.^2 + TAUYY.^2)/2 + swell2(swell2(TAUXY,1),2).^2);
            LAMA = LAMA + lam;
        end
        err_pl(it_tstep)       = max(TII(:)./yield_stress-1);
        % === PLASTICITY
        ...
        if it_tstep>250
            ...
            if mod(it_tstep,1e3)==1 | it_tstep==251
                figure(1)
