...
@parallel function compute_8!(Pf::Data.Array, Rho_X::Data.Array, Res_Phi::Data.Array, Phi::Data.Array, Res_Pf::Data.Array, Rho_s::Data.Array, X_s::Data.Array, Phi_old::Data.Array, Rho_X_old::Data.Array, ∇V_ρ_x::Data.Array, dt_Pf::Data.Number, dtp::Data.Number, _dx::Data.Number, _dy::Data.Number)

	@inn(Pf)      = @inn(Pf) + dt_Pf*@all(Res_Pf)
	@all(Rho_X)   = @all(Rho_s)*@all(X_s)
	@all(Res_Phi) = ( (1.0-@all(Phi))*@all(Rho_X) - (1.0-@all(Phi_old))*@all(Rho_X_old) )/dtp + @all(∇V_ρ_x)   # CONSERVATION OF MASS OF MgO EQUATION
...
@parallel function compute_11!(τII::Data.Array, Res_Vx::Data.Array, Res_Vy::Data.Array, τ_xx::Data.Array, τ_yy::Data.Array, TmpS2::Data.Array, Ptot::Data.Array, τ_xy::Data.Array, _dx::Data.Number, _dy::Data.Number)

	@all(τII)    = sqrt( 0.25*pow(@all(τ_xx)-@all(τ_yy),2) + pow(@all(TmpS2),2) )
...
@views function PT_HMC()
    ...
    Rho_s          .= Data.Array(Rho_s)
    Rho_f          .= Data.Array(Rho_f)
    X_s            .= Data.Array(X_s)
    Rho_s_ini      .= Data.Array(Rho_s_ini)
    X_s_ini        .= Data.Array(X_s_ini)
    ...
    while timeP < time_tot
        ...
        if itp==1
    		...
    		# Phi_ini    .= Phi_old
    		# Phi        .= Phi_old
            @parallel compute_2!(Rho_X_ini, Phi_old, Phi_ini, Phi, Rho_s_ini, X_s_ini, Rho_X_old, ϕ_ini)
    	end
     ...
     #    Rho_t_old      .= Rho_f_old.*Phi_old .+ Rho_s_old.*(1.0 .- Phi_old)
    	...
    	while err_M>tol && it_tstep<itmax
         ...
         #    Pf[2:end-1,2:end-1] .= Pf[2:end-1,2:end-1] .+ dt_Pf.*Res_Pf
        	...
        	# Phi        .= Phi .+ dtp.*Res_Phi
        	@parallel compute_8!(Pf, Rho_X, Res_Phi, Phi, Res_Pf, Rho_s, X_s, Phi_old, Rho_X_old, ∇V_ρ_x, dt_Pf, dtp, _dx, _dy)

        	# Stokes
        	swell2!(TmpX, (1.0.-Phi).*Rho_X, 1)
        	swell2!(TmpY, (1.0.-Phi).*Rho_X, 2)
        	∇V_ρ_x     .= diff(TmpX.*Vx, dims=1)./dx .+ diff(TmpY.*Vy, dims=2)./dy

        	swell2!(TmpX, Rho_t, 1)
        	swell2!(TmpY, Rho_t, 2)
        	∇V_ρ_t     .= diff(TmpX.*Vx, dims=1)./dx .+ diff(TmpY.*Vy, dims=2)./dy
        	...
        	swell2!(TmpS1, τ_xy,  1)
        	swell2!(TmpS2, TmpS1, 2)
        	# τ_xy  .= Data.Array(τ_xy)
        	# TmpS1 .= Data.Array(TmpS1)
        	# TmpS2 .= Data.Array(TmpS2)

        	# comp_11
        	# τII        .= sqrt.( 0.25.*(τ_xx-τ_yy).^2 .+ TmpS2.^2)
        	...
        	@parallel compute_11!(τII, Res_Vx, Res_Vy, τ_xx, τ_yy, TmpS2, Ptot, τ_xy, _dx, _dy)
        	# power-law
        	if n_exp>1
        		# comp_12
        		...
        		@parallel compute_12!(Eta_iter, Eta_pl, Eta, Eta_m, τII, σ_ref, n_exp, relax)
        	end
        	# comp_13
