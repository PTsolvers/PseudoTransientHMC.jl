...
@views function PT_HMC()
    ...
    τ_relax         = η_m   / K_s
    τ_kinetic       = 1.0/5.0 * 1/3*τ_f_dif_ϕ
    τ_deform        = 0.5 * 1.0*τ_f_dif_ϕ
    ε_bg            = 1.0 / τ_deform
    Da              = ε_bg/(P_ini/η_m)   # Re-scaling of Da (LAMBDA_4)
    # Numerical resolution
    nx              = 160 - 1 # -1 due to overlength of array nx+1, multiple of 16 for optimal GPU perf
    ny              = 160 - 1 # -1 due to overlength of array ny+1, multiple of 16 for optimal GPU perf
    ...
    #     Phi_ini[:,2:end-1] .= Phi_ini[:,2:end-1] .+ 0.4.*(Phi_ini[:,3:end].-2.0.*Phi_ini[:,2:end-1].+Phi_ini[:,1:end-2])
    # end
    Pert            = Data.Array((2.5 .* Phi_ini .- Phi_ini) .* exp.(.-(Xc2./rad_a).^2 .- (Yc2./rad_b).^2))
    ...
    while timeP < time_tot
        ...
        if do_save && (itp % nsave == 0 || itp==1)
            matwrite("output_$(runid)_$(nx)x$(ny)/pt_hmc_Atg_$(itp).mat",
