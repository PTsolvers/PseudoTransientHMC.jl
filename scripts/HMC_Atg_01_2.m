...
% Schmalholz et al., submitted to Solid Earth
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
PHI_INI(sqrt((X_ROT-2).^2./rad_a^2+(Y_ROT+6).^2./rad_b^2)<1) = 2.5*phi_ini;  % Fluid pressure perturbation
PHI_INI(sqrt((X_ROT+2).^2./rad_a^2+(Y_ROT-6).^2./rad_b^2)<1) = 2.5*phi_ini;  % Fluid pressure perturbation
for smo=1:3 % Smooting of initial porosity
    Ii               = [2:nx-1];
...
VX_ps           =  eb*X2D_vx;                   % Pure shear, shortening in x
VY_ps           = -eb*Y2D_vy;                   % Pure shear, extension in y
VX_ss           =  eb*Y2D_vx;                   % Simple shear, shearing in x
VY_ss           = 0*Y2D_vy;                     % Simple shear, zero velocity in y
VX              = 0.5 * (VX_ps + VX_ss);
VY              = 0.5 * (VY_ps + VY_ss);
...
while time < time_tot
    ...
    if mod(itp,2e1)==1 | itp==1
        ...
        Time_phys_sec       = 0.01^2/(1e-23/1e-3*1e11);
