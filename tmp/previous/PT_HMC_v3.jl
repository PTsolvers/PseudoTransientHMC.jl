...
@views function PT_HMC()
    ...
    # Visu
    println("iter tot = $it")
    swell2s!(Vx_f, q_f_X./(Rho_f[1:end-1,:]./Phi[1:end-1,:].+Rho_f[2:end,:]./Phi[2:end,:]).*2.0, 1)
    swell2s!(Vy_f, q_f_Y./(Rho_f[:,1:end-1]./Phi[:,1:end-1].+Rho_f[:,2:end]./Phi[:,2:end]).*2.0, 2)
    swell2s!(Vx_f_Ptot, q_f_X_Ptot./(Rho_f[1:end-1,:]./Phi[1:end-1,:].+Rho_f[2:end,:]./Phi[2:end,:]).*2.0, 1)
    swell2s!(VY_f_Ptot, q_f_Y_Ptot./(Rho_f[:,1:end-1]./Phi[:,1:end-1].+Rho_f[:,2:end]./Phi[:,2:end]).*2.0, 2)
