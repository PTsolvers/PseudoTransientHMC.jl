...
slope_ini_ana       = (P_ini*ones(nx,  ny)-p_min_ana)./p_max_ana*rho_s_up;
rho_s_max           = RHO_s_LU(1) ;%max(RHO_s_LU);
rho_s_min_ana       = min(RHO_s_LU);
rho_s_dif_ana       = rho_s_max-rho_s_min_ana;
p_reaction_ana      = 7.85*1e8*Pini_Pappl;
rho_f_min_ana       = min(Rho_f_LU);
fac_ana             = 6e3;
x_max               = max(X_LU);
x_min_ana           = min(X_LU);
...
RHO_S           = (-tanh( 5e2*(Pf-p_reaction_ana) ) *rho_s_dif_ana/2 + rho_s_dif_ana/2 + rho_s_min_ana + slope_ana ) /rho_0;
RHO_F           = (rho_f_min_ana-fac_ana*p_min_ana.^(1/20) + fac_ana*(Pf).^(1/20)) /rho_0;
X_S             =  -tanh( 5e2*(Pf-p_reaction_ana) ) *x_dif_ana/2 + x_dif_ana/2 + x_min_ana;
RHO_S_ini       = (-tanh( 5e2*(P_ini*ones(nx,  ny)-p_reaction_ana) ) *rho_s_dif_ana/2 + rho_s_dif_ana/2 + rho_s_min_ana + slope_ini_ana ) /rho_0;
X_S_ini         =  -tanh( 5e2*(P_ini*ones(nx,  ny)-p_reaction_ana) ) *x_dif_ana/2 + x_dif_ana/2 + x_min_ana;
