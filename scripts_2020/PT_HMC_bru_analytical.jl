...
@parallel function compute_6!(q_f_X::Data.Array, q_f_Y::Data.Array, q_f_X_Ptot::Data.Array, q_f_Y_Ptot::Data.Array, para_cx::Data.Array, para_cy::Data.Array, Pf::Data.Array, Ptot::Data.Array, dx::Data.Number, dy::Data.Number)
...
# @parallel function compute_7L!(∇q_f::Data.Array, Res_Pf::Data.Array, q_f_X::Data.Array, q_f_Y::Data.Array, Rho_t::Data.Array, Rho_t_old::Data.Array, ∇V_ρ_t::Data.Array, dtp::Data.Number, dx::Data.Number, dy::Data.Number)
...
@parallel function compute_7!(∇q_f::Data.Array, Res_Pf::Data.Array, Rho_s::Data.Array, Rho_f::Data.Array, X_s::Data.Array, q_f_X::Data.Array, q_f_Y::Data.Array, Rho_t::Data.Array, Rho_t_old::Data.Array, ∇V_ρ_t::Data.Array, Pf::Data.Array, SlopeA::Data.Array, p_reactA::Data.Number, rho_s_difA::Data.Number, rho_s_minA::Data.Number, ρ_0::Data.Number, rho_f_minA::Data.Number, facA::Data.Number, p_minA::Data.Number, x_difA::Data.Number, x_minA::Data.Number, dtp::Data.Number, dx::Data.Number, dy::Data.Number)
    @all(∇q_f)   = @d_xi(q_f_X)/dx + @d_yi(q_f_Y)/dy
    @all(Res_Pf) = -@all(∇q_f) - (@inn(Rho_t) - @inn(Rho_t_old))/dtp - @inn(∇V_ρ_t) # CONSERVATION OF TOTAL MASS EQUATION

    @all(Rho_s)  = (-tanh( 5e2*(@all(Pf)-p_reactA) )*rho_s_difA/2.0 + rho_s_difA/2.0 + rho_s_minA + @all(SlopeA))/ρ_0
    @all(Rho_f)  = (rho_f_minA-facA*p_minA^(1/20) .+ facA*@all(Pf)^(1/20))/ρ_0
    @all(X_s)    =  -tanh( 5e2*(@all(Pf)-p_reactA) )*x_difA/2.0 + x_difA/2.0 + x_minA
    return
end

@parallel function compute_8!(Pf::Data.Array, Rho_X_ϕ::Data.Array, Res_Phi::Data.Array, Phi::Data.Array, Res_Pf::Data.Array, Rho_s::Data.Array, X_s::Data.Array, Phi_old::Data.Array, Rho_X_old::Data.Array, ∇V_ρ_x::Data.Array, dt_Pf::Data.Number, dtp::Data.Number)
...
@parallel function compute_9!(∇V::Data.Array, ε_xx::Data.Array, ε_yy::Data.Array, ε_xy::Data.Array, Ptot::Data.Array, Vx::Data.Array, Vy::Data.Array, Phi::Data.Array, Pf::Data.Array, Lam::Data.Array, dx::Data.Number, dy::Data.Number)
...
@parallel function compute_11!(τII::Data.Array, Res_Vx::Data.Array, Res_Vy::Data.Array, τ_xx::Data.Array, τ_yy::Data.Array, τ_xyn::Data.Array, Ptot::Data.Array, τ_xy::Data.Array, dx::Data.Number, dy::Data.Number)
...
@parallel_indices (ix,iy) function compute_12!(Eta_iter::Data.Array, Eta_pl::Data.Array, Eta::Data.Array, Eta_m::Data.Array, τII::Data.Array, σ_ref::Data.Number, n_exp::Data.Number, relax::Data.Number)
    ...
    if (ix<=size(Eta_pl,1)   && iy<=size(Eta_pl,2))    Eta_pl[ix,iy]   = exp(log(Eta_pl[ix,iy])*relax + log(Eta_iter[ix,iy])*(1.0-relax)) end
...
@views function PT_HMC_()
    ...
    # Pf_tmp          = zeros(nx, ny)
    Rho_s           = Data.Array( (-tanh.( 5e2*(Pf.-p_reactA) )*rho_s_difA/2.0 .+ rho_s_difA/2.0 .+ rho_s_minA .+ SlopeA)/ρ_0 )
    Rho_f           = Data.Array( (rho_f_minA-facA*p_minA^(1/20) .+ facA*Pf.^(1/20))/ρ_0 )
    ...
    while timeP < time_tot
    	...
    	while err_M>tol && it_tstep<itmax
            ...
            @parallel compute_6!(q_f_X, q_f_Y, q_f_X_Ptot, q_f_Y_Ptot, para_cx, para_cy, Pf, Ptot, dx, dy)
            # @parallel compute_7L!(∇q_f, Res_Pf, q_f_X, q_f_Y, Rho_t, Rho_t_old, ∇V_ρ_t, dtp, dx, dy)
            # if it_tstep % 10 == 0 || it_tstep==1 # Look up for densities
            #     Pf_tmp .= Array( Pf )
            #     Rho_s  .= Data.Array( itp1.(Pf_tmp)./ρ_0 )
            #     Rho_f  .= Data.Array( itp2.(Pf_tmp)./ρ_0 )
            #     X_s    .= Data.Array( itp3.(Pf_tmp)      )
            # end
            @parallel compute_7!(∇q_f, Res_Pf, Rho_s, Rho_f, X_s, q_f_X, q_f_Y, Rho_t, Rho_t_old, ∇V_ρ_t, Pf, SlopeA, p_reactA, rho_s_difA, rho_s_minA, ρ_0, rho_f_minA, facA, p_minA, x_difA, x_minA, dtp, dx, dy)
            # Porosity evolution
            @parallel compute_8!(Pf, Rho_X_ϕ, Res_Phi, Phi, Res_Pf, Rho_s, X_s, Phi_old, Rho_X_old, ∇V_ρ_x, dt_Pf, dtp)
            # Stokes
            ...
            @parallel compute_9!(∇V, ε_xx, ε_yy, ε_xy, Ptot, Vx, Vy, Phi, Pf, Lam, dx, dy)
            @parallel compute_10!(τ_xx, τ_yy, τ_xy, Eta, ε_xx, ε_yy, ε_xy)
            @parallel swell2!(TmpS1, τ_xy,  1)
            @parallel swell2!(τ_xyn, TmpS1, 2)
            @parallel compute_11!(τII, Res_Vx, Res_Vy, τ_xx, τ_yy, τ_xyn, Ptot, τ_xy, dx, dy)
            # power-law
