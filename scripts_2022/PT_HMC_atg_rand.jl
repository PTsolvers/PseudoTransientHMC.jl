...
@views function PT_HMC_()
    ...
    nviz       = 20
    do_restart = false
    irestart   = 100 # Step to restart from if do_reatart
    # read in mat file
    ...
    Pini_Pappl      = P_ini/12.73e8 # Dimensionless ratio of abritrary model-P_ini to P_ini in applicable Pa-values; necessary for Look-up table
    ...
    τ_relax         = η_m   / K_s
    τ_kinetic       = 1.0/5.0/3.0/3.0/3.0/3.0 * 1/3*τ_f_dif_ϕ
    τ_deform        = 0.5/7.0/6.0 * 1.0*τ_f_dif_ϕ
    ε_bg            = 1.0 / τ_deform
    ...
    Vy_f            = zeros(nx  , ny  )
    # POROSITY INITIAL #############################################################################
    # Elliptical porosity field ####################################################################
    #Phi_ini         = ϕ_ini*ones(nx, ny)
    # Phi_ini[sqrt.((X_rot .- 2.0).^2 ./ rad_a.^2 .+ (Y_rot .+ 6.0).^2 ./ rad_b.^2) .< 1.0] .= 2.5*ϕ_ini  # Porosity pertubation
    # Phi_ini[sqrt.((X_rot .+ 2.0).^2 ./ rad_a.^2 .+ (Y_rot .- 6.0).^2 ./ rad_b.^2) .< 1.0] .= 2.5*ϕ_ini  # Porosity pertubation
